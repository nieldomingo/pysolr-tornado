...
class SolrTornadoError(Exception):
    pass
...
class SolrTornado(object):
    ...
    def _send_request(self, method, path='', body=None, headers=None, files=None):
        ...
        #    raise SolrTornadoError("Unable to send HTTP method '{0}.".format(method))
        ...
        try:
            ...
            if err.code == 599:
                ...
                raise SolrTornadoError(error_message % params)
        ...
        if int(resp.code) != 200:
            ...
            self.log.error(error_message, extra={'data': {'headers': resp.headers,
                                                          'response': resp.body}})
            raise SolrTornadoError(error_message)

        raise Return(force_unicode(resp.body))
    ...
    def _select(self, params):
        ...
        if len(params_encoded) < 1024:
            ...
            resp_body = yield self._send_request('get', path)
            ...
            resp_body = yield self._send_request('post',
                                                 path,
                                                 body=params_encoded,
                                                 headers=headers)
    ...
    def _update(self, message, clean_ctrl_chars=True, commit=True, waitFlush=None, waitSearcher=None):
        ...
        resp_body = yield self._send_request('post', path, message, {'Content-type': 'text/xml; charset=utf-8'})
        raise Return(resp_body)
    ...
    def add(self, docs, commit=True, boost=None, commitWithin=None, waitFlush=None, waitSearcher=None):
        ...
        resp_body = yield self._update(m,
                                       commit=commit,
                                       waitFlush=waitFlush,
    ...
    def commit(self, waitFlush=None, waitSearcher=None, expungeDeletes=None):
        ...
        resp_body = yield self._update(msg,
                                       waitFlush=waitFlush,
                                       waitSearcher=waitSearcher)
        raise Return(resp_body)
